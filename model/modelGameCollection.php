...
{
    $bdd = dbConnect();
    $gamerInformationQuery = $bdd->query('SELECT * FROM JOUEUR WHERE Email_Joueur=\'' . $emailGamer . '\'');
    $returnAllGamerInformationQuery = $gamerInformationQuery->fetchAll(PDO::FETCH_ASSOC);
...
function isItPlayerPassword($emailGamer, $password)
{
    $bdd = dbConnect();
    isAPlayerInDataBase($emailGamer);
    $gamerInformationQuery = $bdd->query('SELECT * FROM JOUEUR WHERE Email_Joueur=\'' . $emailGamer . '\'');
    $returnAllGamerInformationQuery = $gamerInformationQuery->fetchAll(PDO::FETCH_ASSOC);
    if (password_verify($password, $returnAllGamerInformationQuery[0]['Mdp_Joueur'])) {
        return true;
    }
    var_dump($returnAllGamerInformationQuery);
    return false;
}
