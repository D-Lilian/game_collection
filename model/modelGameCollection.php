<?php
 ...
 function getAllGames($emailGamer){
    $bdd = dbConnect();
    $allGameQuery = $bdd->query('SELECT * FROM JEU WHERE Id_Jeu NOT IN (SELECT Id_Jeu FROM COLLECTION WHERE Email_Joueur="'.$emailGamer.'")');
...
function getGamesWithSearch($filter){
    $bdd = dbConnect();
    $allGameQuery = $bdd->query('SELECT * FROM JEU WHERE Nom_Jeu LIKE \'%'.$filter.'%\' AND Id_Jeu NOT IN (SELECT Id_Jeu FROM COLLECTION WHERE Email_Joueur="'.$filter.'");');
    $returnAllGameQuery = $allGameQuery->fetchAll(PDO::FETCH_ASSOC);
    return $returnAllGameQuery;
}
...
function getGamePlayer($emailPlayer,$idGame){
    $bdd = dbConnect();
    $allGamerInformationQuery = $bdd->query('SELECT * FROM JEU INNER JOIN COLLECTION ON COLLECTION.Id_Jeu=JEU.Id_Jeu WHERE Email_Joueur=\''.$emailPlayer.'\' AND JEU.Id_Jeu='.$idGame.';');
    $returnAllGamerInformationQuery = $allGamerInformationQuery->fetchAll(PDO::FETCH_ASSOC);
